...
public class UpdateUserForm {
    ...
    public void validate(List<User> list, UpdateUserForm updateUserForm, Errors error) {
        for (User user : list) {
            if (Objects.equals(user.getUsername(), updateUserForm.getUsername())) {
                continue;
            }
            if (Objects.equals(user.getEmail(), updateUserForm.getEmail())) {
                error.rejectValue("email", "email", "Email "
                        + updateUserForm.getEmail() + " đã được sử dụng vui lòng nhập email khác");

            }
            if (Objects.equals(user.getPhoneNumber(), updateUserForm.getPhoneNumber())) {
                error.rejectValue("phoneNumber", "phoneNumber"
                        , "Số điện thoại " + updateUserForm.getPhoneNumber()
                                + " đã được sử dụng vui lòng nhập số điện thoại khác");
            }
        }
    }
